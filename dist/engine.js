...
exports.applyBerryIfTriggered = applyBerryIfTriggered;
exports.printHit = printHit;
const damage_1 = require("./damage");
// Build initial FighterState with computed max HP via a dummy calc call
function initFighter(gen, set) {
    ...
    const fake = (0, damage_1.damageSummary)(gen, set, set, 'Tackle'); // not used, but Pokemon() computes stats
...
function applyBerryIfTriggered(holder, beforeHPPercent, afterRollPercents, berryName) {
    const consumedOn = [];
    let note;
    if (holder.consumedItems.has(berryName))
        return { consumedOn, note };
    ...
    for (let i = 0; i < afterRollPercents.length; i++) {
        if (afterRollPercents[i] <= threshold)
            consumedOn.push(i);
    }
...
function printHit(attacker, defender, moveName, gen) {
    const sum = (0, damage_1.damageSummary)(gen, attacker.set, defender.set, moveName);
