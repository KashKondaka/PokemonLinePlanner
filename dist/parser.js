...
exports.parseShowdownBlock = parseShowdownBlock;
exports.parseShowdownTeamsFile = parseShowdownTeamsFile;
exports.parseEnemyCompactLines = parseEnemyCompactLines;
exports.parseEnemyCompact = parseEnemyCompact;
// Map IV labels to keys
const IV_KEY_MAP = {
    'HP': 'hp', 'Atk': 'atk', 'Def': 'def', 'SpA': 'spa', 'SpD': 'spd', 'Spe': 'spe'
};
function parseShowdownBlock(block) {
    const lines = block.split(/\r?\n/).map(l => l.trim()).filter(Boolean);
    if (!lines.length)
        throw new Error('Empty Showdown block');
    const header = lines.shift();
    const m = header.match(/^(.+?)(?:\s*@\s*(.+))?$/);
    if (!m)
        throw new Error(`Invalid set header: ${header}`);
    const species = m[1].trim();
    const item = m[2]?.trim();
    const set = { species, item, level: 50, moves: [], evs: {}, ivs: {} };
    for (const line of lines) {
        if (/^Ability:/i.test(line))
            set.ability = line.split(':')[1].trim();
        else if (/^Level:/i.test(line))
            set.level = parseInt(line.split(':')[1].trim(), 10);
 ...
 */
function parseShowdownTeamsFile(text) {
 ...
 */
function parseEnemyCompactLines(text) {
    const sets = [];
...
function parseEnemyCompact(line) {
